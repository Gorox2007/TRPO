...
Примечание для Этапа 4:
- для ускорения выборки кандидатов добавлен индекс `idx_users_status_updated_at`;
- для ускорения получения фото добавлен индекс `idx_user_photos_active_lookup`;
- для будущих фильтров по профилям добавлен индекс `idx_users_city_gender`.
