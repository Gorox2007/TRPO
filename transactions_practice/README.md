...
Решение вынесено в отдельную папку и реализует 3 требуемых транзакционных сценария для схемы интернет-магазина.
...
Порт PostgreSQL на хост намеренно не пробрасывается, чтобы избежать конфликтов (например, когда локально уже занят `5432`).
